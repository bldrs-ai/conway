...
bldrs-ai/create's proof fixtures carry OCCT-computed reference measures
per fixture (`out/proof/proof.json`, e.g. `box-fillet-r8` →
`occtArea: 3891.398…`). Loading the same STEP through conway and summing
triangle areas + bbox from `dumpToOBJ` gives a cheap parity number that
caught the sphere, torus, and fillet families in turn. Worth promoting
into the Phase 2 regression harness as a second signal beside digests.
...
`Logger` dedups on the exact message string, so a message that
interpolates nothing collapses its whole family to one row in
`errors.csv` and hides its own size. Sizing work (#495/#496) added
`Logger.error(message, expressID?)` — new extraction errors should pass
the entity ID rather than baking it into the string, which keeps dedup
working *and* makes the family attributable.
