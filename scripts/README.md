...
| `m3_worker_pool.mjs` | Geometry parallel efficiency (#394 M3): N `worker_threads`, each with its own `IfcAPI` and its own wasm linear memory (the no-SharedArrayBuffer shape production can actually ship), each claiming a `SetGeometryShard` and pumping to completion. Reports per-shard geometry times, the **duplication factor** (summed shard CPU ÷ single-worker time — the term that separates "the partition costs work" from "the partition is unbalanced"), per-worker wasm/V8 heaps and process peak RSS, and checks the union of the shards against the single-worker load byte-for-byte. `NO_PAYLOAD_DIGEST=1` drops the harness's own per-shard SHA-256, which otherwise rides on the duplication it is measuring. Measured on D3D at 0.864 (N=2) and 0.476 (N=4) — see `design/new/load-performance-ledger.md` §11. The union check also caught sharding changing D3D's geometry, which it could never previously *report*: its failure path assigned to a read-only ESM namespace and crashed before printing anything |
| `spin_calibrate.mjs` | The box's own parallel ceiling: a register-bound integer spin loop across N **processes**, no allocation and no memory traffic. Run it immediately before *and* after any efficiency measurement — an efficiency without one is uninterpretable, and a contended box has previously inverted a verdict. Reads ~0.997 at N=2 and ~0.99 at N=4 on an idle 4-core box |
