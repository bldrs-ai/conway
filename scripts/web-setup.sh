#!/usr/bin/env bash
# Canonical bootstrap for a fresh checkout: install deps (yarn Berry), check out
# submodules, and build the TypeScript so `yarn lint` / `yarn typecheck` /
# `yarn build-incremental` work.
#
...
#
# The EMSDK + conway-geom WASM build is NOT here (it is slow and only needed to
# *run* tests/geometry); use scripts/setup or .claude/hooks/wasm-setup.sh for
# that. Steps are idempotent and gated on per-step completion stamps so a killed
# run resumes instead of redoing finished work.
...
# makes `yarn test` / `yarn build-codex-MT` work in a web session.
...
# 1. node_modules — Berry immutable install, retried with a per-attempt timeout
# that resumes from the global cache. Immutable so a lockfile mismatch fails
...
# typecheck/build need the checkout (not its WASM build).
