 ...
 */
function generateDeltaCSV(csvPath1, csvPath2, outputCsvPath) {
  const data1 = readDataFromCsv(csvPath1);
  const data2 = readDataFromCsv(csvPath2);

  const deltas = computeDeltas(data1, data2);
  writeDataToCsv(deltas, outputCsvPath);
}
 ...
 */
function writeDataToCsv(data, csvFilename) {
  // Define CSV header (columns for the delta CSV)
  const csvHeader = [
    'timestamp',
    'loadStatus1',
    'loadStatus2',
 ...
 */
function computeDeltas(data1, data2) {
  ...
  // First pass: for each filename in data1, see if present in data2
  for (const filename of Object.keys(data1ByFile)) {
    const entry1 = data1ByFile[filename];
    const entry2 = data2ByFile[filename];

    if (entry2) {
      // Both files have this filename => compute actual deltas
      ...
      // Present in data1, missing in data2 => partial row
      deltas.push({
        timestamp: entry1.timestamp,
        loadStatus1: entry1.loadStatus,
        loadStatus2: 'N/A',
  ...
  // Second pass: for each filename in data2 that wasn't in data1
...
// node delta.js oldCsvPath newCsvPath outCsvPath
if (require.main === module) {
  if (process.argv.length !== 5) {
    console.error(`Usage: node ${process.argv[1]} <run_name1.csv> <run_name2.csv> <output_csv_filename>`);
    process.exit(1);
  }

  const [,, runName1, runName2, outputCsv] = process.argv;
  generateDeltaCSV(runName1, runName2, outputCsv);
}
