...
| Whether two perf numbers may be subtracted at all: the measured 13.7% cross-run noise floor, why the runner label hides three CPU models, why the PMU is masked and instruction counts are unobtainable, and the in-job pairing that makes the rc timing delta a measurement instead of a lead | [design/new/perf-run-comparability.md](design/new/perf-run-comparability.md) |
