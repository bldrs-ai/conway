...
| Which memory machinery (scratch arena, geometry budget/residency, adaptive source-window residency) actually covers which geometry path — audited matrix for STEP and IFC, corrected site counts, the stated policy for new tessellation entry points, and D3D's measured per-path allocation profile (CSG is 98.8 % of allocator traffic; extrusion 1.2 %) | [design/new/geometry-memory-coverage.md](design/new/geometry-memory-coverage.md) |
