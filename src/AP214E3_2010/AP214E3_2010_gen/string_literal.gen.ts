...
export  class string_literal extends simple_string_expression {
  ...
  public get the_value() : string {
    if ( this.the_value_ === void 0 ) {
      this.the_value_ = this.extractString( 0, 0, 4, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === string_literal.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for string_literal" )
      }
