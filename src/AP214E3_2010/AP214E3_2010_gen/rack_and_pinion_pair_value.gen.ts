...
export  class rack_and_pinion_pair_value extends pair_value {
  ...
  public get actual_displacement() : number {
    if ( this.actual_displacement_ === void 0 ) {
      this.actual_displacement_ = this.extractNumber( 2, 1, 1, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === rack_and_pinion_pair_value.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for rack_and_pinion_pair_value" )
      }
