...
export  class composite_text_with_extent extends composite_text {
  ...
  public get extent() : planar_extent {
    if ( this.extent_ === void 0 ) {
      this.extent_ = this.extractElement( 2, 2, 3, false, planar_extent )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === composite_text_with_extent.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for composite_text_with_extent" )
      }
