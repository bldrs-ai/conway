...
export  class datum extends shape_aspect {
  ...
  public get identification() : string {
    if ( this.identification_ === void 0 ) {
      this.identification_ = this.extractString( 4, 4, 1, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === datum.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for datum" )
      }
