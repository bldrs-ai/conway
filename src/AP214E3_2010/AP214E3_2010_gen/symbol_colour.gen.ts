...
export  class symbol_colour extends StepEntityBase< EntityTypesAP214 > {
  ...
  public get colour_of_symbol() : colour {
    if ( this.colour_of_symbol_ === void 0 ) {
      this.colour_of_symbol_ = this.extractElement( 0, 0, 0, false, colour )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === symbol_colour.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for symbol_colour" )
      }
