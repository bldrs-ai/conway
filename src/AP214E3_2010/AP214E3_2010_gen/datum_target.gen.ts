...
export  class datum_target extends shape_aspect {
  ...
  public get target_id() : string {
    if ( this.target_id_ === void 0 ) {
      this.target_id_ = this.extractString( 4, 4, 1, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === datum_target.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for datum_target" )
      }
