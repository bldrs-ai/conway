...
export abstract class action_assignment extends StepEntityBase< EntityTypesAP214 > {
  ...
  public get assigned_action() : action {
    if ( this.assigned_action_ === void 0 ) {
      this.assigned_action_ = this.extractElement( 0, 0, 0, false, action )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === action_assignment.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for action_assignment" )
      }
