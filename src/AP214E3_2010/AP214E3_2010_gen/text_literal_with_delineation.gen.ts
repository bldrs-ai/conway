...
export  class text_literal_with_delineation extends text_literal {
  ...
  public get delineation() : string {
    if ( this.delineation_ === void 0 ) {
      this.delineation_ = this.extractString( 6, 6, 3, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === text_literal_with_delineation.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for text_literal_with_delineation" )
      }
