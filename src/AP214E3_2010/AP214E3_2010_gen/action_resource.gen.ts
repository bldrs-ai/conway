...
export  class action_resource extends StepEntityBase< EntityTypesAP214 > {
  ...
  public get kind() : action_resource_type {
    if ( this.kind_ === void 0 ) {
      this.kind_ = this.extractElement( 3, 0, 0, false, action_resource_type )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === action_resource.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for action_resource" )
      }
