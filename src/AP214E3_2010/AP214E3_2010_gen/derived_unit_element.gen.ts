...
export  class derived_unit_element extends StepEntityBase< EntityTypesAP214 > {
  ...
  public get unit() : named_unit {
    if ( this.unit_ === void 0 ) {
      this.unit_ = this.extractElement( 0, 0, 0, false, named_unit )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === derived_unit_element.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for derived_unit_element" )
      }
