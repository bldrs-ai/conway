...
export  class point_on_surface_pair extends kinematic_pair {
  ...
  public get pair_surface() : surface {
    if ( this.pair_surface_ === void 0 ) {
      this.pair_surface_ = this.extractElement( 5, 5, 2, false, surface )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === point_on_surface_pair.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for point_on_surface_pair" )
      }
