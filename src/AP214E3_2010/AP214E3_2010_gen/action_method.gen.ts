...
export  class action_method extends StepEntityBase< EntityTypesAP214 > {
  ...
  public get consequence() : string {
    if ( this.consequence_ === void 0 ) {
      this.consequence_ = this.extractString( 2, 0, 0, false )
    }
  ...
  public get purpose() : string {
    if ( this.purpose_ === void 0 ) {
      this.purpose_ = this.extractString( 3, 0, 0, false )
    }
  ...
  constructor(
    ...
    if ( multiReference !== void 0 ) {
      ...
      const localReference =
        multiReference.find( ( item ) => item.typeID === action_method.expectedType )

      if ( localReference === void 0 ) {
        throw new Error( "Couldn't find multi-element reference for action_method" )
      }
